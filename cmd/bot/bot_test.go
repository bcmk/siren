...
func TestUpdateStatus(t *testing.T) {
	w := newTestWorker()
	w.createDatabase()
	if n, _ := w.processStatusUpdates([]lib.StatusUpdate{{"a", lib.StatusOnline}}, 18); n == 0 {
		t.Error("unexpected status update")
	}
	if n, _ := w.processStatusUpdates([]lib.StatusUpdate{{"a", lib.StatusOffline}}, 19); n != 0 {
		t.Error("unexpected status update")
	}
	if n, _ := w.processStatusUpdates([]lib.StatusUpdate{{"a", lib.StatusOffline}}, 20); n != 0 {
		t.Error("unexpected status update")
	}
	if n, _ := w.processStatusUpdates([]lib.StatusUpdate{{"a", lib.StatusOnline}}, 21); n != 0 {
		t.Error("unexpected status update")
	}
	if w.onlineModelsCount() != 1 {
		t.Error("wrong online models count")
	}
	if n, _ := w.processStatusUpdates([]lib.StatusUpdate{{"a", lib.StatusOnline}}, 22); n != 0 {
		t.Error("unexpected status update")
	}
	if n, _ := w.processStatusUpdates([]lib.StatusUpdate{{"a", lib.StatusNotFound}}, 23); n != 0 {
		t.Error("unexpected status update")
	}
	if w.onlineModelsCount() != 1 {
		t.Error("wrong online models count")
	}
	if n, _ := w.processStatusUpdates([]lib.StatusUpdate{{"a", lib.StatusNotFound}}, 24); n != 0 {
		t.Error("unexpected status update")
	}
	if w.confirmedStatus("a") != lib.StatusOnline {
		t.Error("wrong active status")
	}
	if n, _ := w.processStatusUpdates([]lib.StatusUpdate{{"a", lib.StatusNotFound}}, 29); n == 0 {
		t.Error("unexpected status update")
	}
	if w.confirmedStatus("a") != lib.StatusNotFound {
		t.Error("wrong active status")
	}
	if n, _ := w.processStatusUpdates([]lib.StatusUpdate{{"a", lib.StatusOnline}}, 30); n == 0 {
		t.Error("unexpected status update")
	}
	if n, _ := w.processStatusUpdates([]lib.StatusUpdate{{"a", lib.StatusOnline}}, 31); n != 0 {
		t.Error("unexpected status update")
	}
	if n, _ := w.processStatusUpdates([]lib.StatusUpdate{{"a", lib.StatusUnknown}}, 32); n != 0 {
		t.Error("unexpected status update")
	}
	if n, _ := w.processStatusUpdates([]lib.StatusUpdate{{"a", lib.StatusUnknown}}, 33); n != 0 {
		t.Error("unexpected status update")
	}
	w.processStatusUpdates([]lib.StatusUpdate{{"a", lib.StatusOffline}}, 34)
	if n, _ := w.processStatusUpdates([]lib.StatusUpdate{{"a", lib.StatusNotFound}}, 35); n != 0 {
		t.Error("unexpected status update")
	}
	if n, _ := w.processStatusUpdates([]lib.StatusUpdate{{"a", lib.StatusOnline}}, 37); n != 0 {
		t.Error("unexpected status update")
	}
	if n, _ := w.processStatusUpdates([]lib.StatusUpdate{{"a", lib.StatusOffline}}, 37); n != 0 {
		t.Error("unexpected status update")
	}
	if n, _ := w.processStatusUpdates([]lib.StatusUpdate{{"a", lib.StatusOnline}}, 41); n != 0 {
		t.Error("unexpected status update")
	}
	if n, _ := w.processStatusUpdates([]lib.StatusUpdate{{"a", lib.StatusOffline}}, 42); n != 0 {
		t.Error("unexpected status update")
	}
	if n, _ := w.processStatusUpdates([]lib.StatusUpdate{{"a", lib.StatusOnline}}, 48); n != 0 {
		t.Error("unexpected status update")
	}
	w.processStatusUpdates([]lib.StatusUpdate{{"a", lib.StatusNotFound}}, 49)
	w.processStatusUpdates([]lib.StatusUpdate{{"a", lib.StatusNotFound}}, 50)
	w.processStatusUpdates([]lib.StatusUpdate{{"a", lib.StatusOnline}}, 50)
	w.processStatusUpdates([]lib.StatusUpdate{{"a", lib.StatusNotFound}}, 52)
	w.processStatusUpdates([]lib.StatusUpdate{{"b", lib.StatusOnline}}, 53)
	w.processStatusUpdates([]lib.StatusUpdate{{"b", lib.StatusOffline}}, 54)
	if w.confirmedStatus("b") != lib.StatusOnline {
		t.Error("wrong active status")
	}
	w.processStatusUpdates([]lib.StatusUpdate{{"b", lib.StatusOnline}}, 55)
	if w.confirmedStatus("b") != lib.StatusOnline {
		t.Error("wrong active status")
	}
	...
	w.processStatusUpdates([]lib.StatusUpdate{{"a", lib.StatusOnline}}, 56)
	if count := w.onlineModelsCount(); count != 2 {
		t.Errorf("wrong online models count: %d", count)
	}
	w.cfg.OfflineNotifications = true
	if n, _ := w.processStatusUpdates([]lib.StatusUpdate{{"a", lib.StatusOffline}}, 57); n != 0 {
		t.Error("unexpected status update")
	}
	if w.confirmedStatus("a") != lib.StatusOnline {
		t.Error("wrong active status")
	}
	if n, _ := w.processStatusUpdates([]lib.StatusUpdate{{"a", lib.StatusOffline}}, 68); n == 0 {
		t.Error("unexpected status update")
	}
...
func TestUpdateStatusFromNotFoundToOnline(t *testing.T) {
	w := newTestWorker()
	w.createDatabase()
	if n, _ := w.processStatusUpdates([]lib.StatusUpdate{{"a", lib.StatusNotFound}}, 18); n != 0 {
		t.Error("unexpected status update")
	}
	if n, _ := w.processStatusUpdates([]lib.StatusUpdate{{"a", lib.StatusNotFound}}, 24); n == 0 {
		t.Error("unexpected status update")
